...
const ORDERED: any = { bayer: 0, noise: 1, lines: 2 };
...
const getBlueNoise = () => {
  ...
  const find = (state: any, highest: any) => {
    let best = 0;
...
const diffuse = (pixels: any, cols: any, rows: any, kernel: any, levels: any, rgb: any, grade: any) => {
...
const DitherVeil = ({
  ...
  useEffect(() => {
    ...
    const program = (fragment: any, uniforms: any) =>
    ...
    const updateDiffusion = (cell: any, s: any) => {
      ...
      const grade = (v: any) => Math.pow(Math.min(1, Math.max(0, (v - 0.5) * s.contrast + 0.5 + s.brightness)), 1.6);
